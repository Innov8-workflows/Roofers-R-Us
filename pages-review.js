 ...
 *
 * The one card Dave texts a customer the day a job finishes. One job: get them
...
module.exports = function writeReviewPage({ write, G }) {
  ...
  /* ---------------------------------------------------------- the primary ask
   * Dave has no Google Business Profile yet, so there is nowhere for a customer
  ...
  const CSS = [
    ...
    'body{background:#101318;color:#13213B;font:400 16px/1.55 Inter,system-ui,-apple-system,"Segoe UI",Roboto,Arial,sans-serif;display:flex;align-items:flex-start;justify-content:center;min-height:100vh}',
    '.card{width:100%;max-width:520px;background:#fff;padding-bottom:4px}',
    '.hd{position:relative;padding:34px 24px 26px;text-align:center;background:#101318;overflow:hidden}',
    /* opacity and scrim both eased 40% on 2026-09-07 so the work shows through.
       Contrast of the strapline against the ground was measured after, not assumed. */
    '.hd-bg{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.64}',
    '.hd-sc{position:absolute;inset:0;background:linear-gradient(180deg,rgba(16,19,24,.36),rgba(16,19,24,.53))}',
    '.hd-in{position:relative}',
    '.logo{width:210px;height:86px;margin:0 auto 14px;background:url("' + asset('logo.webp') + '") center/contain no-repeat;display:block}',
    '.hd p{color:#CBD5E8;font-size:14px}',
    '.body{padding:28px 24px 0;text-align:center}',
    '.stars{display:flex;gap:5px;justify-content:center;margin-bottom:14px}',
    '.eyebrow{font:700 11.5px/1 Inter,sans-serif;letter-spacing:.16em;text-transform:uppercase;color:#3D4A5C;margin-bottom:9px}',
    'h1{font:800 30px/1.15 "Plus Jakarta Sans",Inter,sans-serif;color:#13213B;margin-bottom:12px}',
    '.thanks{color:#3B4A66;font-size:16px;max-width:40ch;margin:0 auto}',
    '.ask{margin:24px 24px 0;background:#F5F7FB;border:1px solid #E1E6EF;border-radius:14px;padding:22px 20px;text-align:center}',
    '.ask h2{font:800 19px/1.25 "Plus Jakarta Sans",Inter,sans-serif;color:#13213B;margin-bottom:9px}',
    '.ask p{color:#5C6B86;font-size:14.5px;margin-bottom:18px}',
    '.btn{display:flex;align-items:center;justify-content:center;gap:11px;width:100%;background:#556378;color:#fff;font:700 17px/1 "Plus Jakarta Sans",Inter,sans-serif;padding:18px 20px;border-radius:12px;text-decoration:none;box-shadow:0 6px 18px rgba(85,99,120,.28)}',
    '.btn-note{display:block;margin-top:10px;font-size:12.5px;color:#8A97AD}',
    '.steps{margin:26px 24px 0;counter-reset:s;text-align:left}',
    '.steps h2{font:800 15px/1 "Plus Jakarta Sans",Inter,sans-serif;color:#13213B;margin-bottom:14px;text-align:center}',
    '.step{counter-increment:s;position:relative;padding-left:40px;margin-top:13px;color:#3B4A66;font-size:14.5px}',
    '.step::before{counter-increment:none;content:counter(s);position:absolute;left:0;top:-1px;width:26px;height:26px;border-radius:50%;background:#EEF2F8;color:#3D4A5C;font:800 13px/26px Inter,sans-serif;text-align:center}',
    '.put{margin:28px 24px 0;border-top:1px solid #E9EDF5;padding-top:22px;text-align:center}',
    '.put h2{font:800 16px/1 "Plus Jakarta Sans",Inter,sans-serif;color:#13213B;margin-bottom:8px}',
    '.put p{color:#5C6B86;font-size:14px;margin-bottom:15px}',
    '.put-btns{display:flex;gap:10px}',
    '.pbtn{flex:1;display:flex;align-items:center;justify-content:center;gap:8px;border:1px solid #C2CBDA;border-radius:11px;padding:13px 10px;text-decoration:none;color:#13213B;font:600 14.5px/1 Inter,sans-serif}',
    '.pbtn .icon{width:18px;height:18px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}',
    '.ft{margin-top:26px;padding:18px 24px 24px;border-top:1px solid #E9EDF5;text-align:center;color:#8A97AD;font-size:12px}',
    '.ft a{color:#5C6B86}',
    '.reassure{display:block;margin-bottom:6px;color:#5C6B86}',
  ...
  const html = '<!doctype html>\n<html lang="en-GB">\n<head>\n' +
    ...
    '    <video class="hd-bg" autoplay muted loop playsinline preload="auto" poster="' + asset('hero-poster.webp') + '" aria-hidden="true">' +
    '<source src="' + asset('hero.mp4') + '" type="video/mp4"></video>\n' +
    '    <div class="hd-sc"></div>\n    <div class="hd-in">\n' +
    '      <span class="logo" role="img" aria-label="' + esc(T.BUSINESS) + '"></span>\n' +
    '      <p>Roofing across ' + esc(T.TOWN) + ', Redditch and ' + esc(T.AREA2) + '</p>\n    </div>\n  </div>\n\n' +
    '  <div class="body">\n' +
