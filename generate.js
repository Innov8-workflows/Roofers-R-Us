/* Roofers R Us Ltd - multi-page build-out.
 *
 *   node generate.js                              writes _site/   (domain root)
 *   STAGING_BASE=Roofers-R-Us node generate.js    writes _staging/ for the GitHub
 *                                                 Pages preview, noindex on every page
 *
 * Same shape as the Midland Roof Shield generator (2026-09-06): the site-kit has
 * no multi-page engine, so this is project-level.
 *
 ...
 *   _site/ rather than rebuilt here. Its markup, section order, hero video and
 *   transformation video are untouched and nothing is re-encoded. On the way
 *   through, build-pages.js injects a canonical, og: tags and the LocalBusiness
 *   schema (the kit template emits none) and swaps every base64 data: URI back
 *   to its real file path - same bytes, addressed differently.
 *
 * CSS url() WARNING
 ...
 *   and 404s. Every url here is root-absolute via B for exactly that reason.
 *
...
function buildCss() {
  ...
  /* the small roof mark, used by the nav, drawer and footer on every page */
  const rootBlock = ':root{' + varName('mark.webp') + ':url("' + asset('mark.webp') + '")}\n';
  return rootBlock + core + '\n' + EXTRA_CSS;
}

/* Layout for the page types the demo body does not have. Built only from
   tokens the direction already owns - no new colours. */
const EXTRA_CSS = `
...
.rv-card{background:#fff;border:1px solid var(--paper-line);border-radius:var(--r-lg);padding:20px;margin-top:14px}
.rv-card b{display:block;color:var(--on-paper)}
.rv-card small{color:var(--on-paper-muted)}
.rv-card p{color:var(--on-paper-2);margin-top:10px}
.rv-stars{color:var(--gold);letter-spacing:2px;font-size:15px;margin-top:6px}
`;
...
function waText(where) { return tok(SITE.whatsapp.opener(where)); }
function waHref(where) { return 'https://wa.me/' + T.PHONE_WA + '?text=' + encodeURIComponent(waText(where)); }
...
function nav() {
  ...
  <div class="nav-in">
    <a class="brand" href="${B}" aria-label="${esc(T.BUSINESS)} home">
      <span class="logo-mark" style="background-image:var(${varName('mark.webp')})" role="img" aria-label="${esc(T.BUSINESS)} logo"></span>
      <span class="brand-txt"><b>${esc(T.BUSINESS_SHORT)}</b><span>Roofing for three generations</span></span>
    </a>
...
<div class="drawer" id="drawer" role="dialog" aria-modal="true" aria-label="Menu">
  <div class="drawer-top">
    <a class="brand" href="${B}">
      <span class="logo-mark" style="background-image:var(${varName('mark.webp')})" aria-hidden="true"></span>
      <span class="brand-txt"><b>${esc(T.BUSINESS_SHORT)}</b><span>Roofing for three generations</span></span>
    </a>
  ...
  <div class="wrap">
    <div class="foot-grid">
      <div class="foot-brand">
        <span class="logo-mark" style="background-image:var(${varName('mark.webp')})" role="img" aria-label="${esc(T.BUSINESS)}"></span>
        <p>Roof repairs, re-roofing, flat roofs, chimneys and guttering across ${esc(T.TOWN)}, Redditch and ${esc(T.AREA2)}. Roofing for three generations.</p>
        <div class="socials">
          <a href="${esc(T.FACEBOOK)}" target="_blank" rel="noopener" aria-label="Facebook"><svg viewBox="0 0 24 24">${FB_PATH}</svg></a>
...
${areaLinks}
      ...
      <div>
        ...
        <ul>
          ...
          <li><a href="${esc(T.GOOGLE)}" target="_blank" rel="noopener">Google reviews</a></li>
          <li>${esc(SITE.contact.addressOneLine)}</li>
        </ul>
      </div>
    </div>
    <div class="foot-bar">
      <small>${esc(C.name)}, registered in England and Wales, company ${esc(C.number)}.<br>
...
<link href="https://fonts.googleapis.com/css2?${DIR.fonts}&display=swap" rel="stylesheet">
...
/* One LocalBusiness node, referenced by @id everywhere else. NO aggregateRating:
   see content/site.js reviews for why. The address carries only what the
   submission gave - a unit and a street - so the empty fields are omitted
   rather than guessed. */
function bizNode() {
  const a = SITE.contact.address;
  const address = { '@type': 'PostalAddress', streetAddress: a.line1 + ', ' + a.line2, addressCountry: a.country };
  if (a.town) address.addressLocality = a.town;
...
module.exports = {
  ...
  bizNode, webPage, faqSchema, BIZ_ID, NAV, WA_PATH, FB_PATH, waHref, waText, ICON,
};
